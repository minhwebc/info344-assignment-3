...
namespace WebRole2
{
    sealed class MyTrieNode : BurstNavigable
    {
        ...
        private void GetWords(BurstNavigable temp, HashSet<string> result, string resultString)
        {
            if (result.Count >= 10)
                return;
            if (temp == null)
        ...
        private void GetWords(BurstNavigable temp, char[] word, HashSet <string> result, int start, string resultString)
        {
            if (temp == null)
                return;
